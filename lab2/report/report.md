...
1. Вычисляем значение функции $e^{-x}$ при помощи разложения в ряд телора по формуле: $\sum\limits_ {n=0}^{\infin}\frac{(-1^n)x^n}{n!}$ Данный ряд является рядом Лейбницевского типа поэтому погрешность можно оценить так: $|R_n| < b_ {n+1}$. Далее учтя, что $e^x = \frac{1}{e^{-x}}$ вычислим значение второй функции. 
